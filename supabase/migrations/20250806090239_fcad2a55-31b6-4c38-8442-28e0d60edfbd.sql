...
INSERT INTO public.subsidies (
    code, title, description, eligibility_criteria, amount_min, amount_max, deadline,
...
SELECT 
    ...
    CASE 
        WHEN amount ~ '^[0-9.,]+' THEN CAST(regexp_replace(split_part(amount, ' ', 1), '[^0-9.]', '', 'g') AS numeric)
        ELSE NULL
    END as amount_min,
    NULL as amount_max,
    deadline,
...
BEGIN
    ...
    VALUES (
        ...
        CASE 
            WHEN NEW.amount ~ '^[0-9.,]+' THEN CAST(regexp_replace(split_part(NEW.amount, ' ', 1), '[^0-9.]', '', 'g') AS numeric)
            ELSE NULL
        END,
        NULL,
        NEW.deadline,
