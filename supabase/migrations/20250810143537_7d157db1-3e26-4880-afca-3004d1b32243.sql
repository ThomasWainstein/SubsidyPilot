...
-- 2) Auto-fix all policies that call auth.* or current_setting()
DO $$
DECLARE
  r RECORD;
  new_qual TEXT;
  ...
  -- Helper function to wrap common auth/current_setting calls with (select ...)
  FUNCTION wrap_calls(expr TEXT) RETURNS TEXT LANGUAGE plpgsql AS $f$
  BEGIN
...
BEGIN
  ...
  LOOP
    new_qual  := wrap_calls(r.qual);
    new_check := wrap_calls(r.with_check);
    ...
    create_sql :=
      'CREATE POLICY ' || quote_ident(r.polname) || ' ON ' || fq_table || ' ' ||
    ...
    RAISE NOTICE 'Fixing policy %.%: %', r.tablename, r.polname, create_sql;

    EXECUTE 'DROP POLICY ' || quote_ident(r.polname) || ' ON ' || fq_table || ';';
    EXECUTE create_sql;
  END LOOP;
END $$;