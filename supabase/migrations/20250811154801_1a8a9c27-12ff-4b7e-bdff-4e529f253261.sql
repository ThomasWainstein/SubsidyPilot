...
SELECT 
  ...
  COUNT(CASE WHEN rsp.content_length > 1000 THEN 1 END) as quality_pages,
  AVG(rsp.content_length) as avg_content_length,
...
-- Replace blanket subsidies access with role-based access
...
CREATE POLICY "consultants_can_read_subsidies" 
ON public.subsidies 
FOR SELECT 
USING (public.has_role(auth.uid(), 'consultant') OR public.has_role(auth.uid(), 'admin'));

CREATE POLICY "farmers_can_read_open_subsidies" 
ON public.subsidies 
FOR SELECT 
USING (
...
CREATE POLICY "consultants_can_read_subsidies_structured" 
ON public.subsidies_structured 
FOR SELECT 
USING (public.has_role(auth.uid(), 'consultant') OR public.has_role(auth.uid(), 'admin'));

CREATE POLICY "farmers_can_read_active_subsidies_structured" 
ON public.subsidies_structured 
FOR SELECT 
USING (
  public.has_role(auth.uid(), 'farmer') AND 
