-- Phase 2: Async Document Processing Infrastructure
...
BEGIN
  ...
  IF NEW.bucket_id = 'farm-documents' AND TG_OP = 'INSERT' THEN
    ...
    SELECT fd.*, f.user_id INTO document_record, farm_user_id
    ...
    IF document_record.id IS NOT NULL THEN
      ...
      PERFORM net.http_post(
        url := current_setting('app.supabase_url') || '/functions/v1/async-document-processor',
        ...
        body := jsonb_build_object(
          ...
          'fileUrl', document_record.file_url,
          'fileName', document_record.file_name,
          'clientType', COALESCE(document_record.category::text, 'farm'),
          'documentType', document_record.category::text,
          'userId', farm_user_id
        )
      );
      
      RAISE LOG 'Triggered async processing for document %', document_record.id;
    END IF;
  END IF;
