...
export const PhaseValidationDiagnostics = () => {
  ...
  const handleRunDiagnostics = async () => {
    setIsRunning(true);
    try {
      const results = await runImmediateDiagnostics();
      setDiagnosticResults(results);
    } catch (error) {
      console.error('Diagnostics failed:', error);
      setDiagnosticResults([{ test: 'System Error', success: false, error: error.message }]);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>🚀 Phase 1A Critical System Validation</CardTitle>
          <CardDescription>
            Enhanced diagnostics with UUID fixes and improved document fetching to resolve blocking issues
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Button 
              ...
              className="w-full"
            >
              {isRunning ? 'Running Diagnostics...' : 'Run Phase 1A Diagnostics'}
            </Button>
            ...
            <Button 
              ...
              variant="default"
            >
              {isCriticalRunning ? 'Running Critical Tests...' : 'Run FIXED Critical Tests'}
            </Button>
          </div>
      ...
      {diagnosticResults.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Standard Diagnostic Results</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {criticalResults.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-green-600">🚀 FIXED Critical Validation Results</CardTitle>
          </CardHeader>
          <CardContent>
