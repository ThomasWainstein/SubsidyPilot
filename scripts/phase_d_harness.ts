...
// Optional: HARNESS_BUCKET (default "farm-documents"), ENABLE_PHASE_D must be set in your function secrets
...
async function uploadAndProcess(fpath: string, mime: string) {
  ...
  const up = await anon.storage.from(BUCKET).upload(storagePath, file, { contentType: mime, upsert: true });
  if (up.error) throw up.error;

  // Call edge function
  const fnUrl = `https://${PROJECT_REF}.functions.supabase.co/extract-document-data`;
  console.log(`⚡ Calling function: ${fnUrl}`);
  ...
  while (Date.now() < deadline) {
    ...
    record = data?.[0];
    if (record?.status && ["completed", "success", "succeeded"].includes(record.status)) break;
    if (record?.status === "failed") break;

    await new Promise((r) => setTimeout(r, 1500));
  }

  if (!record) throw new Error("No extraction row found in time.");
