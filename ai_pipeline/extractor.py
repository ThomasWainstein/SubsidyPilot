...
class AIExtractor:
    ...
    async def process_records_async(
        ...
        """Process provided raw records without fetching from Supabase."""

        self.logger.info("🚀 Starting async AI extraction on provided records")
        self.stats.start_time = time.time()
        ...
        try:
            ...
            if not records:
                self.logger.info("✅ No records provided")
                return self._finalize_stats()

            self.logger.info(f"📄 Received {len(records)} records")
            self.stats.total_processed = len(records)

            tasks = [
                self._process_single_log_with_retry(record, i + 1)
            ...
            self.logger.error(f"❌ Critical error in record processing: {e}")
            self.logger.error(traceback.format_exc())
            return self._finalize_stats()
...
- Maintain original French terminology and phrasing"""
    ...
    def process_records(self, records: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Synchronous wrapper for :meth:`process_records_async`."""

        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            stats = loop.run_until_complete(self.process_records_async(records))
            return asdict(stats)
        finally:
            loop.close()
